...
sys.setrecursionlimit(100000)  # Python의 재귀 한도를 늘려줍니다.

dice = list(map(int, input().split()))  # 주사위의 입력값
piece = [0] * 4  # 현재 말의 위치

arr = [i + 1 for i in range(21)] + [21] + [i + 1 for i in range(5)] + [20, 29, 30, 25, 32, 25]  # 다음에 갈 위치 저장
score = [2 * i for i in range(21)] + [13, 16, 19, 25, 30, 35, 28, 27, 26, 22, 24]  # 윷놀이판 엔트리의 점수
